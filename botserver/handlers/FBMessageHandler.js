...
const send_wolfram_query_result = (senderID, numberOfLocations, query) => {
    var wolfram_query_send = wolfram_query.wolfram_query_random(query, numberOfLocations, function (result, resultType) {
...
module.exports = (event) => {
    ...
    ChatLogDB.logMessage({senderID: senderID, senderName: senderName, message: fbUserMessage});
    ...
    getWitAPIData((witData) => {
        ...
        if (witData.entities) {
            if (witData.entities.wolfram_search_query) { //wolfram_search_query
                ...
                var result_wolfram = wolfram_query.wolfram_query_random(fbUserMessage, 0,function (result, resultType) {
                    ...
                    case witEntities.intents.get_place_info:
                        if (witData.entities.location) {
                            if (witData.entities.datetime) {
                                var result_wolfram = wolfram_query.wolfram_query_random(fbUserMessage, witData.entities.location.length,function (result, resultType) {
                                    console.log(fbUserMessage);
                                    console.log(result);
                                    replyMessageQuery(senderID, result, resultType)
                                });
                    ...
                    default:
                        if (witData.entities.location) {
                            ...
                            if (witData.entities.datetime){
                                ...
                                var result_wolfram = wolfram_query.wolfram_query_intent(intent, location, witData.entities.location.length, function (result, resultType) {
            ...
            var result_wolfram = wolfram_query.wolfram_query_random(fbUserMessage, function (result, resultType) {
                console.log(fbUserMessage);
                console.log(result);
                //sendTextMessage(senderID, {"text": result});
                replyMessageQuery(senderID, result, resultType)
